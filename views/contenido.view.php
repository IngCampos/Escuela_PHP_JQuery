...
<body>
	...
	codigo en caso de ser alumno
		...
		$statement = $conexion->prepare('SELECT materias.Nombre, materias.Grado, materias.Descripcion, clases.Hora FROM clases INNER JOIN materias ON clases.Id_materia=materias.Id WHERE clases.Id_maestro = :Id ORDER BY clases.Hora');
		...
		while($materias!=null){
			echo "<tr><td>".$materias["Hora"]."</td><td>".$materias["Nombre"]."</td><td>".$materias["Grado"]."</td><td><a>Asistencias</a></td><td><a>Tareas</a></td></tr>";
			$materias = $statement->fetch();
		}
		echo "</table>";
